...
static unsigned int g_dwLineWidth; //显示屏： 一行数据占据多少字节
static unsigned int g_dwPixelWidth; //显示屏： 一个像素占多少字节
...
static int FBDeviceInit(void)
{
	...
	g_dwScreenSize = g_tFBVar.xres * g_tFBVar.yres * g_tFBVar.bits_per_pixel / 8; //显示一张图片共占多少字节
